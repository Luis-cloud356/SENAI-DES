...
const totalGasto = async(req, res) => {
     const lista = await db.query("select cliente_id, sum(filmes.preco) as 'Total'from locacoes inner join filmes on locacoes.filme_id=filmes.id group by cliente_id order by locacoes.id asc;");
    res.json(lista[0]).end();
}
module.exports = {
     listarClientes,
